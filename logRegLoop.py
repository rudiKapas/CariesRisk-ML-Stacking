...
def evaluate_model(k_folds):
    ...
    # 2) Compute all FPR points and average the TPR
    ...
    # Store results for each candidate parameter combination from GridSearchCV
    # (We use the same best-estimator metrics for all candidates here.)
    results = []
    ...
    for i in range(total_models):
        ...
        results.append({
            ...
            # Store the macro-average ROC curve
...
# Flatten results (since we now return multiple candidate results per k-fold)
...
print("Training completed. Results saved to logistic_regression_results.csv")
...
plt.title('ROC-AUC Curve for Best Logistic Regression Model (Macro-Average)')
plt.legend()
plt.grid(True)
plt.savefig('roc_auc_curve_logistic.png')
plt.show()
print("ROC-AUC Curve saved as roc_auc_curve_logistic.png")
