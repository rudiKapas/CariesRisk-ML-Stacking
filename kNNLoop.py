...
n_neighbors_list = [3, 5, 10, 15, 50, 100]
...
def evaluate_model(k_folds, n_neighbors, weights, metric):
    ...
    for train_index, test_index in kfold.split(X_train):
        ...
        y_proba = knn.predict_proba(X_test_fold)
...
results = Parallel(n_jobs=-1)(delayed(evaluate_model)(k_folds, n_neighbors, weights, metric)
                              for k_folds, n_neighbors, weights, metric in
                              product(k_values, n_neighbors_list, weights_list, metric_list))
...
print("Training completed. Results saved to knn_results.csv")
...
plt.title('ROC-AUC Curve for Best KNN Model')
plt.legend()
plt.grid(True)
